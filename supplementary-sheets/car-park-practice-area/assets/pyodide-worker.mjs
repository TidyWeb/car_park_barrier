...
self.onmessage = async ({ data }) => {
  ...
  try {
    ...
    for (const [name, contents] of Object.entries(files)) {
      if (!/^[A-Za-z0-9_-]+\.py$/.test(name)) throw new Error(`Invalid virtual filename: ${name}`);
      pyodide.FS.writeFile(name, String(contents));
    }
    pyodide.globals.set("__learner_code", data.code);
    pyodide.globals.set("__browser_files", Object.keys(files));
    await pyodide.runPythonAsync(`
import builtins
import sys
from pyodide.ffi import run_sync

for _browser_file in __browser_files:
    sys.modules.pop(_browser_file.removesuffix(".py"), None)
